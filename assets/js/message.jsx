...
export class MessageWindowContainer extends React.Component {
    ...
    connectToNotificationChannel = () => {
        if (this.state.user) {
            var notificationWebsocket = new ReconnectingWebSocket(ws_scheme + '://' + window.location.host + ":5443/notifications/" + this.state.user.notificationChannelLabel + "/?token=" + localStorage.token)
            ...
            $.ajax({
                ...
                success: function (messageThreadData) {
                    if (messageThreadData.results[0]) {
                        ...
                        theMessageThreadData.websocket = new ReconnectingWebSocket(ws_scheme + '://' + window.location.host + ":5443/chat-messages/" + theMessageThreadData.channelLabel + "/?token=" + localStorage.token)
...
//            chat_socket.send(JSON.stringify(message));
    ...
    $.ajax({
        ...
        success: function (messageThreadData) {
            messageThreadData.websocket = new ReconnectingWebSocket(ws_scheme + '://' + window.location.host + ":5443/chat-messages/" + messageThreadData.channelLabel + "/?token=" + localStorage.token)
    ...
    connectToWebsocket = (websocketType, websocketLabel) => {
        var theSocket = new ReconnectingWebSocket(ws_scheme + '://' + window.location.host + ":5443/" + websocketType + "/" + websocketLabel + "/?token=" + localStorage.token)
...
export class MessagePage extends React.Component {
    ...
    connectToNotificationChannel = (notificationChannelLabel) => {
        ...
        var notificationWebsocket = new ReconnectingWebSocket(ws_scheme + '://' + window.location.host + ":5443/notifications/" + notificationChannelLabel + "/?token=" + localStorage.token)
        notificationWebsocket.onmessage = (message) => {
            var messageData = JSON.parse(message.data);
            if (messageData.text == "openPrivateTextChannel") {
                this.connectToThread(messageData.privateChannelId)
    ...
    sendNotificationToWakeupPrivateChannel = (receiverNotificationChannelLabel, privateChannel) => {
        var receiverNotificationWebsocket = new ReconnectingWebSocket(ws_scheme + '://' + window.location.host + ":5443/notifications/" + receiverNotificationChannelLabel + "/?token=" + localStorage.token)
  ...
  render() {
            if (this.state.thread != undefined) {
                if (this.state.thread.channelLabel != undefined) {
                    ...
                    var websocketUrl = ws_scheme + '://' + window.location.host + ":5443/chat-messages/" + this.state.thread.channelLabel + "/?token=" + localStorage.token
